...
class Robstride:
    ...
    def _read_frame(self, msg_id: int, timeout_s: int) -> bytes:
        message: can.CAN_Message = self.can_transport.recv(can_id=msg_id, timeout_s=timeout_s)
        if message != None:
            return message.data

if __name__ == "__main__":
    pcan_transport = can.PCAN(channel=PCANBasic.PCAN_USBBUS1, baud_rate=can.CAN_Baud_Rate.CAN_BAUD_1_MBS)
    robstride = Robstride(host_can_id=0x1, motor_can_id=0x1, can_transport=pcan_transport)
